...
import { AlertTriangle, Clock, Shield, ArrowRight, MessageSquare, User, Building, Target } from "lucide-react";
...
export const ThreatFeed = ({ alerts, isLoading = false, onViewDetails }: ThreatFeedProps) => {
  ...
  // Display entities specifically for each threat
  const displayEntities = (alert: ContentAlert) => {
          ...
          {alerts.slice(0, displayCount).map((alert, idx) => (
            <div key={alert.id} className="group">
              <div className="p-4 hover:bg-muted/30 transition-colors">
                ...
                {displayEntities(alert)}
                
                {alert.recommendation && (
                  <div className="mt-1 text-xs text-muted-foreground">
                    <strong>Recommended action:</strong> {alert.recommendation.length > 60 
                      ? `${alert.recommendation.substring(0, 60)}...` 
                      : alert.recommendation}
                  </div>
                )}
