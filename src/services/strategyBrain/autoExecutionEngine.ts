...
export const enableAutoExecution = async (
  ...
  try {
    ...
    console.log(`🤖 Auto-execution enabled for ${entityName}`);
...
export const evaluateForAutoExecution = async (
  ...
  try {
    ...
    if (dailyCount >= config.maxDailyExecutions) {
      console.log(`🚫 Daily execution limit reached for ${entityName}`);
      return false;
    }
    ...
    if (lastExecution && isInCooldown(lastExecution, config.cooldownPeriod)) {
      console.log(`⏱️ Cooldown period active for ${entityName}`);
      return false;
    }

    // Check if strategy type is allowed
    if (!config.allowedStrategyTypes.includes(strategy.type)) {
      console.log(`🚫 Strategy type ${strategy.type} not allowed for auto-execution`);
      return false;
    }
    ...
    if (metrics.successProbability < thresholds.minSuccessProbability) {
      console.log(`🚫 Success probability too low: ${metrics.successProbability}`);
      return false;
    }

    if (metrics.riskLevel > thresholds.maxRiskLevel) {
      console.log(`🚫 Risk level too high: ${metrics.riskLevel}`);
      return false;
    }

    if (metrics.resourceRequirement > thresholds.maxResourceRequirement) {
      console.log(`🚫 Resource requirement too high: ${metrics.resourceRequirement}`);
      return false;
    }

    if (metrics.confidenceScore < thresholds.minConfidenceScore) {
      console.log(`🚫 Confidence score too low: ${metrics.confidenceScore}`);
      return false;
    }

    console.log(`✅ Strategy ${strategy.id} approved for auto-execution`);
    return true;
...
export const queueForAutoExecution = async (
  ...
  try {
    ...
    console.log(`📋 Strategy queued for auto-execution: ${queueItem.id}`);
    return queueItem.id;
...
export const processExecutionQueue = async (): Promise<void> => {
  try {
    console.log('🔄 Processing auto-execution queue...');
    ...
    for (const item of queueData) {
      try {
        ...
        console.log(`✅ Auto-executed strategy ${queueItem.strategyId}: ${queueItem.status}`);
