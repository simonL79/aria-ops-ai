...
const titleCase = (v) => (v || '').replace(/_/g, ' ');
const fmtPctT = (n) => (n === null || n === undefined) ? '—' : `${(Number(n) * 100).toFixed(0)}%`;
const buildTimeline = (event) => {
    const items = [];
    if (event.created_at) {
...
const PortalResurfacingEvent = () => {
    ...
    useEffect(() => {
        const load = async () => {
            if (!id)
                return;
            setLoading(true);
            const { data, error } = await supabase.from('eidetic_resurfacing_events')
                .select('*')
                .eq('id', id)
                .maybeSingle();
            if (error)
    ...
    const runAction = async (action, extra) => {
        ...
        setActing(action);
        try {
            const { data, error } = await supabase.rpc('portal_resurfacing_event_action', {
                _event_id: id,
                _action: action,
            ...
            if (data)
                setEvent(data);
            toast.success(action === 'acknowledge' ? 'Event acknowledged'
    ...
    const Row = ({ label, children }) => (<div className="flex flex-col gap-0.5 py-2 border-b border-white/5 last:border-0 sm:flex-row sm:justify-between sm:gap-4">
    ...
    return (<PortalLayout title="Resurfacing Event">
      <div className="mb-4">
      ...
      {loading ? (<div className="text-white/50">Loading…</div>) : notFound || !event ? (<Card className="bg-white/5 border-white/10">
          <CardContent className="p-8 text-center text-white/60">
            This resurfacing event was not found or you don’t have access to it.
          </CardContent>
        </Card>) : (<div className="space-y-6 max-w-3xl">
          {/* Header */}
          <div className="flex flex-wrap items-center gap-2 justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {event.narrative_category && (<Badge variant="outline" className="text-xs">{event.narrative_category}</Badge>)}
              {event.status && (<Badge variant="outline" className="text-xs capitalize">{event.status}</Badge>)}
            </div>
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-5 space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button size="sm" variant="outline" disabled={!!acting || event.acknowledged} onClick={() => runAction('acknowledge')} className="gap-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <Button size="sm" variant="outline" disabled={!!acting} onClick={() => runAction('snooze', { hours: Number(snoozeHours) })} className="gap-1.5">
                    <Clock className="h-3.5 w-3.5"/> Snooze
                  </Button>
                </div>

                {event.status === 'resolved' ? (<Button size="sm" variant="outline" disabled={!!acting} onClick={() => runAction('reopen')} className="gap-1.5">
                    <RotateCcw className="h-3.5 w-3.5"/> Reopen
                  </Button>) : (<Button size="sm" disabled={!!acting} onClick={() => runAction('resolve', resolutionNotes.trim() ? { resolution_notes: resolutionNotes.trim() } : {})} className="gap-1.5">
                    <CheckCircle2 className="h-3.5 w-3.5"/> Resolve
                  </Button>)}
              </div>
              {event.status !== 'resolved' && (<Textarea value={resolutionNotes} onChange={(e) => setResolutionNotes(e.target.value)} placeholder="Optional resolution notes (saved when you resolve this event)…" className="bg-black/20 border-white/10 text-sm" rows={2} disabled={!!acting}/>)}
            </CardContent>
          </Card>

          {/* Excerpt */}
          {event.content_excerpt && (<Card className="bg-white/5 border-white/10">
              <CardContent className="p-5">
                ...
                {event.content_url && (<a href={event.content_url} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs text-orange-400 hover:underline">
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-5">
              ...
              {(() => {
                ...
                if (!items.length)
                    return <div className="text-sm text-white/50">No timeline events yet.</div>;
                return (<ol className="relative border-l border-white/10 ml-1.5 space-y-5">
                    {items.map((it, i) => (<li key={i} className="ml-5">
                        <span className={`absolute -left-[7px] mt-1 h-3 w-3 rounded-full ring-2 ring-[#1C172B] ${toneDot[it.tone]} ${it.future ? 'opacity-50' : ''}`}/>
                        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                          <span className="text-sm font-medium text-white/90">{it.label}</span>
                          {it.future && (<Badge variant="outline" className="text-[10px] py-0">upcoming</Badge>)}
                        </div>
                        ...
                        {it.detail && (<div className="mt-1 text-sm text-white/70 whitespace-pre-wrap">{it.detail}</div>)}
                      </li>))}
                  </ol>);
            })()}
            </CardContent>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-sm text-white/85">
                    ...
                    {event.decay_delta != null && (<span className="ml-2 text-xs text-white/50">(Δ {fmtPct(event.decay_delta)})</span>)}
                  </div>
                </div>
                <div>
                  ...
                  <div className="text-sm text-white/85">
                    ...
                    {event.threat_delta != null && (<span className="ml-2 text-xs text-white/50">(Δ {fmtPct(event.threat_delta)})</span>)}
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-5">
              ...
              <Row label="Detected">
                ...
                {event.created_at && (<span className="ml-2 text-xs text-white/40">
          ...
          {/* Raw metadata */}
          {event.metadata && Object.keys(event.metadata || {}).length > 0 && (<Card className="bg-white/5 border-white/10">
              <CardContent className="p-5">
